...
BaseFrame::BaseFrame()
{
  ...
  QObject::connect(this, SIGNAL(sig_image_received(const QImage&)), base_frame_graphics_view,
                   SLOT(image_received(const QImage&)));
  QObject::connect(this, SIGNAL(sig_tracked_object_changed(const QRectF&)), base_frame_graphics_view,
                   SLOT(tracked_object_changed(const QRectF&)));
  ...
  // Odometry.
  QObject::connect(this, SIGNAL(sig_odom_changed(const QRectF&)), base_frame_graphics_view,
                   SLOT(odometry_changed(const QRectF&)));
  ...
  // Bounding box corrispondente all'odometria.
  sub4 = n.subscribe("odom_bb", 1000, &BaseFrame::odom_bbCB, this);
...
void BaseFrame::image_receivedCB(const sensor_msgs::ImageConstPtr& msg)
{
  if(first_image || base_frame_graphics_view->get_correct_bb())
  {
    ...
    {
      image = QImage((const unsigned char*)(cv_ptr->image.data), cv_ptr->image.cols, cv_ptr->image.rows,
...
void BaseFrame::odom_bbCB(const tld_msgs::BoundingBoxConstPtr& msg)
{
  QRectF rect(msg->x, msg->y, msg->width, msg->height);
  emit sig_odom_changed(rect);
